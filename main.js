...
function showWindow() {
  ...
  //   https://github.com/Dostoapp/dosto-desktop/issues/1429
...
async function createWindow() {
  ...
  mainWindow.on('close', async e => {
     ...
     *
     * issue: https://github.com/signalapp/dosto-desktop/issues/4348
     */
...
function openJoinTheBeta() {
  ...
  shell.openExternal('https://dosto.live');
}

function openReleaseNotes() {
  shell.openExternal(
    `https:dosto.live/v${app.getVersion()}`
  );
}
...
function showSettingsWindow() {
  ...
  const size = mainWindow.getSize();
  const options = {
    width: Math.min(700, size[0]),
    height: Math.max(size[1] - 200, MIN_HEIGHT),
    // height:700,
    // width:700,
    frame: false,
    resizable: false,
