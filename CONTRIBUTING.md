...
for it or creating a new one yourself. You can use also that issue as a place to Dosto
...
You can check [`.nvmrc` in the `development` branch](https://github.com/Dostoapp/Dosto-Desktop/blob/development/.nvmrc) to see what the current version is. If you have [nvm](https://github.com/creationix/nvm)
...
```
git clone https://gitlab.com/biplabb/dosto-desktop-application.git
cd dosto-desktop-application
npm install --global yarn      # (only if you don’t already have `yarn`)
yarn install --frozen-lockfile # Install and build dependencies (this will take a while)
signal-ringrtc-node-zip        # Extract the signal-ringrtc-node file in node_modules with name "ringrtc"
yarn grunt                     # Generate final JS and CSS assets 
OR "yarn grunt --force"        # If you get the error while using  yarn grunt command then you can used 
yarn build:webpack             # Build parts of the app that use webpack (Sticker Creator)
yarn test                      # A good idea to make sure tests run first
yarn start                     # Start Dosto!
```
...
the `Dosto_ENABLE_HTTP` environment variable to a truthy value. On Linux and
macOS, that simply looks like this:

```
Dosto_ENABLE_HTTP=1 yarn start
```
...
empty application. But you can use the information from your production install of Dosto
Desktop to populate your testing application!

First, find your application data:

- macOS: `~/Library/Application Support/Dosto`
- Linux: `~/.config/Dosto`
- Windows 10: `C:\Users\<YourName>\AppData\Roaming\Dosto`

Now make a copy of this production data directory in the same place, and call it
`Dosto-development`. Now start up the development version of the app as normal,
...
directory from `%appData%/Dosto` to `%appData%/Dosto-aliceProfile`.
...
- Please do not submit pull requests for translation fixes. Anyone can update
  the translations in
  [Transifex](https://www.transifex.com/projects/p/Dosto-desktop).
...
of the Dosto experience requires a primary mobile device: contact management,
...
First, build Dosto for Android or iOS from source, and point its service URL to `textsecure-service-staging.whispersystems.org`:

**on Android:** Replace the `Dosto_URL` value in [build.gradle](https://github.com/Dostoapp/Dosto-Android/blob/master/build.gradle)

**on iOS:** Replace the `textSecureServerURL` value in `TSConstants.h`(located in the DostoServiceKit pod)

This task is 1% search and replace, 99% setting up your build environment. Instructions are available for both
the [Android](https://github.com/Dostoapp/Dosto-Android/blob/master/BUILDING.md)
and [iOS](https://github.com/Dostoapp/Dosto-iOS/blob/master/BUILDING.md) projects.

Then you can set up your development build of Dosto Desktop as normal. If you've already
...
```json
{
  "serverUrl": "https://howzit.observanteye.com",
  "serverTrustRoot": "BWh+UOhT1hD8bkb+MFRvb6tVqhoG8YYGCzOd7mgjo8cV"
  "cdn": {
    "0": "https://howzit.observanteye.com:9000/howzit-bucket",
    "2": "https://howzit.observanteye.com:9000"
  }
}
```
